...
pub async fn handle_command(raw: &str, store: &Store) -> Response {
    let req: Request = match serde_json::from_str(raw) {
        Ok(r) => r,
        Err(_) => return Response::Error { status: "error", message: "invalid json".to_string() },
    };
    ...
    match req.cmd.to_uppercase().as_str() {
        ...
        _      => Response::Error { status: "error", message: "unknown command".to_string() },
    }
}
...
async fn cmd_set(req: Request, store: &Store) -> Response {
    let key = match req.key { Some(k) => k, None => return Response::Error { status: "error", message: "missing key".to_string() } };
    let value = match req.value { Some(v) => v, None => return Response::Error { status: "error", message: "missing value".to_string() } };
...
async fn cmd_expire(req: Request, store: &Store) -> Response {
    let key = match req.key { Some(k) => k, None => return Response::Error { status: "error", message: "missing key".to_string() } };
    let secs = match req.seconds { Some(s) => s, None => return Response::Error { status: "error", message: "missing seconds".to_string() }};
    let mut map = store.lock().await;
    match map.get_mut(&key) {
        None => Response::Error { status: "error", message: "key not found".to_string() },
        Some(e) => {
...
async fn cmd_incr_decr(req: Request, store: &Store, delta: i64) -> Response {
    ...
    let current = match map.get(&key) {
        ...
        Some(e) => match e.value.parse::<i64>() {
            Ok(n) => n,
            Err(_) => return Response::Error { status: "error", message: "not an integer".to_string() },
        }
    };
...
async fn cmd_save(store: &Store) -> Response {
    ...
    match serde_json::to_string(&data) {
        Err(_) => Response::Error { status: "error", message: "serialization failed".to_string() },
        Ok(json) => match std::fs::write("dump.json", json) {
            Ok(_) => Response::Ok { status: "ok" },
            Err(e) => Response::Error { status: "error", message: e.to_string() },
        }
    }
}
